...
module.exports = {
        ...
        if (req.wantsJSON) {
            ...
            if (req.body.category == "buy") {
                ...
                if (thatUser.clients.length == 0) {
                    //user's valid order about this stock not exist
                    //console.log("no client");
                    if (thatUser.balances >= 25000 && thatUser.tradeStatus == 1) {
                        //用户余额大于25000， 交易状态为1 （表示账户可进行交易）  无限次T+0交易

                        if (thatUser.balances >= parseFloat(req.body.totalCost)) {
                            //余额是否足够支付
                            ...
                            await User.addToCollection(req.session.usrid, "clients").members(
                                updatedOrder.id
                            );
                    ...
                    } else if (thatUser.balances < 25000 && thatUser.tradeStatus == 1) {
                        //用户余额小于25000， 交易状态为1 （表示账户可进行交易） 可进行4次T+0交易

                        if (
                            thatUser.balances >= parseFloat(req.body.totalCost) &&
                            thatUser.tradeCount > 0
                        ) {
                            //create new order for stock buying
                            var order = await Stock.create(req.body).fetch();
                            ...
                            var updatedUser = await User.updateOne({
                                ...
                                tradeCount: thatUser.tradeCount - 1,
                            });
                            ...
                            return res.json(
                                "Success Buy and tradeStatus" + updatedUser.tradeStatus
                            );
                        } else {
                            return res.json(
                                "Sorry, you may have no enough money or your buying limitation is reaching"
                            );
                        }
                    } else {
                        //余额 < 25000, 交易状态：0
                        return res.json(
                            "user balance is less than 25000 and your account is locked because your count of limitation of trading is already zero. "
                        );
                    }
                } else {
                    //user's valid order exist (valid = 0)
                    ...
                    if (thatUser.balances >= 25000 && thatUser.tradeStatus == 1) {
                        ...
                        if (thatUser.balances >= parseFloat(req.body.totalCost)) {
                            ...
                            var updatedOrder = await Stock.updateOne({
                                ...
                                totalCost:
                                    Math.round(
                                        (order.totalCost + parseFloat(req.body.totalCost)) * 100
                                    ) / 100,
                                price: parseFloat(req.body.price),
                                totalVolume: parseFloat(req.body.volume) + order.totalVolume,
                ...
                if (thatUser.clients.length != 0) {
                    ...
                    if (thatUser.clients[0].totalVolume >= parseFloat(req.body.volume)) {
                        ...
                        if (
                            thatUser.clients[0].totalVolume == parseFloat(req.body.volume)
                        ) {
                            var order = await Stock.findOne(thatUser.clients[0].id);
                            ...
                            var updatedOrder = await Stock.updateOne({
                                ...
                                totalRevenue:
                                    Math.round(
                                        (order.totalRevenue + parseFloat(req.body.totalRevenue)) *
                                        100
                                    ) / 100,
                                price: parseFloat(req.body.price),
                                totalVolume: order.totalVolume - parseFloat(req.body.volume),
                                category: req.body.category,
                                datetime: req.body.datetime,
                            });
                            ...
                            var updatedUser = await User.updateOne({
                                ...
                                balances:
                                    Math.round(
                                        (thatUser.balances + parseFloat(req.body.totalRevenue)) *
                                        100
                                    ) / 100,
                                assets:
                                    Math.round(
                                        (thatUser.assets - parseFloat(req.body.totalRevenue)) * 100
                                    ) / 100,
                            });
                            ...
                            return res.json(
                                "success sell, and all the stock were sold out. "
                            );
                        } else {
                            //如果持有大于要卖股数
                            var order = await Stock.findOne(thatUser.clients[0].id);
                        ...
                        return res.json(
                            "You have no enough stock volume that can be sold. "
                        );
                    }
                } else {
    ...
    //stock marketnews
    market_2: async function (req, res) {
        // const fetch = require("node-fetch");
        if (req.method == "GET") {
            return res.view("stock/market_2");
        }
    },
};
